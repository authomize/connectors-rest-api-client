...
#   timestamp: 2023-03-08T14:38:09+00:00
...
class RequestsBundleSchema(BaseModel):
    ...
    new_groupings: Optional[List[NewGroupingRequestSchema]] = Field(
        None,
        description='The Create Groupings API is used to create groupings.\nA "grouping" is any object that collects multiple accounts or other groupings and can be granted permissions. \nExamples include groups, roles on certain platforms (e.g. AWS), profiles in certain platforms (e.g. SFDC) and more.\n\n',
        title='New Groupings',
    )
    new_permissions: Optional[List[NewPermissionRequestSchema]] = Field(
        None,
        description='A permission (also referred to as an “entitlement”) is a set of privileges, assigned to a specific target. \nExamples include: file permissions, file shares, GCP entitlements, the actual policies assigning access to roles in AWS, and inline policies.\nThe Create Permissions API is used to create a set of privileges, assigned to a specific target.',
