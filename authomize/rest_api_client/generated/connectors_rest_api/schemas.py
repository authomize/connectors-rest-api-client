...
#   timestamp: 2023-04-05T07:49:37+00:00
...
class DistinctMergeRuleRequest(BaseModel):
    distinctIdentityIds: List[str] = Field(
        ..., min_items=2, title='Distinctidentityids'
    )
    actorId: str = Field(..., title='Actorid')
    actorType: ActorType
...
class IdentityUnificationSettingsSchema(BaseModel):
    ...
    shouldMergeIdentitiesBySameEmailPrefixToFirstNameAndLastName: Optional[
        bool
    ] = Field(
        None, title='Shouldmergeidentitiesbysameemailprefixtofirstnameandlastname'
    )
    shouldMergeIdentitiesBySameEmailPrefixToFirstNameInitialAndLastName: Optional[
        bool
    ] = Field(
        None,
...
class InternalSettingListSchema(BaseModel):
    pagination: Optional[Pagination] = Field(
        default_factory=lambda: Pagination.parse_obj(
            {'limit': -1, 'skip': 0, 'total': -1}
        ),
        title='Pagination',
    )
